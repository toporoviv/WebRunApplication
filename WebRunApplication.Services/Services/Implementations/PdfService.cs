...
{
    public async Task<IBaseResponse<FileResultInformation>> GetCurrentTrainingInformationPdfAsync
    (
    ...
    )
    {
        try
        {
            ...
            var title = await trainingTemplateRepository
                .GetAll()
                .FirstOrDefaultAsync(x => x.Id == trainingTemplateId);

            var data = (await indicatorRepository
                    .GetIndicatorsAsync(cancellationToken))
                .Where(x => x.UserId == user.Id)
                .Join(trainingRepository
                        .GetAll()
                        .Where(x => x.TrainTemplateId == trainingTemplateId),
            ...
            var f = new Font(bf, 12, Font.NORMAL);
            ...
            var font2 = new Font(bf, 20, Font.BOLD);
            ...
            cell1.PaddingTop = 5f;
            var bioPhrase = new Phrase();
            var bioChunk = new Chunk("RunApp | Список доступных тренировок по выбранному типу", font2);
            ...
            var cell = new PdfPCell(new Phrase("Список Тренировок", f));
        ...
        {
            logger.LogError($"{nameof(PdfService)}{nameof(GetCurrentTrainingInformationPdfAsync)}: {exception.Message}");
            return new BaseResponse<FileResultInformation>
            {
    ...
    public async Task<IBaseResponse<FileResultInformation>> GetIndicatorsPdfAsync
    (
        string exportData,
    ...
    )
    {
        try
        {
            ...
            var columnCount = typeof(Indicator).GetProperties().Length - 3;
            var table = new PdfPTable(columnCount);
            var cell = new PdfPCell(new Phrase("Показатели тренировок", f))
            {
                Colspan = columnCount,
        ...
        {
            logger.LogError($"{nameof(PdfService)}{nameof(GetIndicatorsPdfAsync)}: {exception.Message}");
            return new BaseResponse<FileResultInformation>
            {
                Description = exception.Message,
                StatusCode = StatusCode.InternalServerError
    ...
    public async Task<IBaseResponse<FileResultInformation>> GetTotalTrainingInformationPdfAsync
    (
        string userLogin,
    ...
    )
    {
        try
        {
            ...
            var user = (await userRepository.GetUsersAsync(cancellationToken))
                .FirstOrDefault(user => user.Login == userLogin);
            ...
            var bioPhrase = new Phrase();
            var bioChunk = new Chunk("RunApp | Данные по общему тренировочному времени", font2);
            bioPhrase.Add(bioChunk);
            bioPhrase.Add(new Chunk(Environment.NewLine));
            ...
            var cell = new PdfPCell(new Phrase("Общее тренировочное время по месяцам", f))
            {
                Colspan = 2,
                HorizontalAlignment = 1 //0=Left, 1=Centre, 2=Right
            };
            ...
            foreach (var trainingTotal in data)
            {
                ...
                var totalTimeMonth = new TimeSpan();

                for (var j = 0; j < trainingTotal.Value.Titles.Count; j++)
                {
                    for (var i = 0; i < trainingTotal.Value.Titles[j].Count; i++)
                    {
                        table.AddCell(new PdfPCell(new Phrase(trainingTotal.Value.Titles[j][i], f)));
        ...
        {
            logger.LogError($"{nameof(PdfService)}{nameof(GetTotalTrainingInformationPdfAsync)}: {exception.Message}");
            return new BaseResponse<FileResultInformation>
            {
                Description = exception.Message,
                StatusCode = StatusCode.InternalServerError
    ...
    public async Task<List<Indicator>> GetUserIndicatorsAsync(string login, CancellationToken cancellationToken)
    {
        // todo: тут юзера может не быть, надо учесть это
        var user = (await userRepository.GetUsersAsync(cancellationToken))
            .FirstOrDefault(x => x.Login == login);

        var userIndicators = (await indicatorRepository
                .GetIndicatorsAsync(cancellationToken))
            .Where(x => x.UserId == user.Id)
            .ToList();
    ...
    public async Task<List<IndicatorViewModel>> GetIndicatorResultsAsync
    (
        string login,
        CancellationToken cancellationToken
    )
    {
        var indicators = await GetUserIndicatorsAsync(login, cancellationToken);
